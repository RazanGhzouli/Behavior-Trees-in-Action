...
def extract_url_repo_name(result):
    ...
    ### formating the file name to desired format with today's date
    date = datetime.today().strftime('%d%m%Y')   
    ### saving the found results 
    f = open("repo_name_plus_URL_" +date+ ".pkl","wb")
...
def download_models(save_path, url_repo_name_dict):
    ...
    for key,value in url_repo_name_dict.items():
        for i in range(len(value)):
            ...
            # open(os.path.join(save_path, os.path.basename(key.replace('/','_')+'_'+(re.search('/(\w*((%20\w)*)?.py)', value[i]).group(1)))) , "wb").write(r.content)
            open(os.path.join(save_path, os.path.basename(key.replace('/','_')+'_'+value.split("/")[-1])) , "wb").write(r.content)
