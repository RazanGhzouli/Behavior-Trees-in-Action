...
from phoenix_msgs.msg import VoiceAction, VoiceGoal
...
def create_root():
    ...
    bumper2bb = py_trees_ros.subscribers.EventToBlackboard(
        name="Bumper2BB",
        topic_name="/rear_bumper",
        variable_name="event_rear_bumper"
    )
    ...
    is_charging_ok = py_trees.blackboard.CheckBlackboardVariable(
        name="On charger?",
        variable_name='on_charge',
    ...
    lounge = py_trees_ros.actions.ActionClient(
        name="lounge",
    ...
    home = py_trees_ros.actions.ActionClient(
        ...
        action_namespace="/move_base",
        action_spec=move_base_msgs.MoveBaseAction,
        action_goal= create_nav_goal(-3.277, -0.155,2.0),
        override_feedback_message_on_running="rotating"
    )
     voice = py_trees_ros.actions.ActionClient(
        name="voice_low_bat",
        action_namespace="/voice_action",
        action_spec=VoiceAction,
        action_goal=voice_msg("my Battree is getting low"),
        override_feedback_message_on_running="rotating"
    )
    ######################### Foyer
    foyer = py_trees.composites.Sequence(name="Foyer")
    is_foyer_requested = py_trees.blackboard.CheckBlackboardVariable(
        name="Nav_Playground?",
        variable_name='event_scan_button',
        expected_value=True
    )
    ...
    #voice_low = py_trees_ros.tutorials.behaviours.SendVoice(name="Flash Green", message="low battery")
    charging = py_trees.timers.Timer("Pause", duration=15.0)
    idle = py_trees.behaviours.Running(name="Idle")

    ################### tree
    root.add_children([topics2bb, priorities])
    topics2bb.add_children([dock2bb, bumper2bb, scan2bb, battery2bb])
    priorities.add_children([battery_check, foyer, nav_home, idle])
    battery_check.add_children([is_battery_ok, voice, auto_dock])
    auto_dock.add_children([is_charging_ok, home, charging])
    nav_home.add_children([is_nav_requested,lounge])
    foyer.add_children([is_foyer_requested, nav_play])
    return root
